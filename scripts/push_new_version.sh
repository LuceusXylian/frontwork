...
for file in "${FILES[@]}"; do
    sed -E -i "s|jsr:@frontwork-org/frontwork@\^?$CURRENT_VERSION|jsr:@frontwork-org/frontwork@^$NEW_VERSION|g" "$file"
done


# Remove the template lockfile: it is fully regenerated by 'deno cache' at the
# end of this script once the new framework version is published to JSR.
...
# Reload cache: regenerate the template's deno.lock with the new framework
# version and integrity hashes.
# --minimum-dependency-age 0: the framework version was published moments ago
# and Deno >=2.9 refuses by default to resolve versions younger than 24 hours.
cd ../frontwork-cli/template
deno cache --reload --minimum-dependency-age 0 --lock=deno.lock src/dependencies.ts
deno cache --reload --minimum-dependency-age 0 --lock=deno.lock src/main.testworker.ts
deno cache --reload --minimum-dependency-age 0 --lock=deno.lock src/main.client.ts
cd ../../scripts
